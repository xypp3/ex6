...
#	endif // ~ !NDEBUG
	...
	// Load textures
	//TODO: load texture(s)
	GLuint textureObjectId = load_texture_2d("assets/floor.jpg");
	...
	while( !glfwWindowShouldClose( window ) )
	{
		...
		Mat44f T = make_translation( Vec3f {0.f, 0.f, -state.camControl.radius} );
		Mat44f world2camera = (((T) * Rx) * Ry);

		Mat44f projection = make_perspective_projection(
			60.f * 3.1315926f / 180.f, /* 60 degrees is a aribtrary value (stated in lecture) */
			fbwidth/((float) fbheight),
			0.1f, /* conventional value (stated in lecture) */
			100.f /* conventional value (stated in lecture) */
		);

		// brackets only clarify how matrix operations are calculated
		...
		// uniform data calls must be AFTER glUseProgram and BEFORE glDrawArrays
		glUniformMatrix4fv( 
			0, /* index in uniform */
